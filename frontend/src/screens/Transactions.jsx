...
export default function Transactions() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["allTxs", bizId, type, search, page],
  ...
  return (
    <div>
      ...
      <div style={{ padding: "12px 16px 0" }}>
        <input style={{ ...S.input, background: C.slate50 }}
      ...
      <div style={{ display: "flex", gap: 6, padding: "10px 16px", overflowX: "auto" }}>
        ...
        {businesses.map(b => (
          <button key={b.id} style={{ ...S.chip, ...(bizId === b.id ? { background: b.color + "18", borderColor: b.color, color: b.color } : {}) }}
            onClick={() => { setBizId(b.id); setPage(1); }}>
            ...
            return (
              <div key={t.id} style={S.txItem}>
                ...
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: C.slate400, background: C.slate100, borderRadius: 4, padding: "1px 5px", display: "inline-block", marginTop: 2 }}>
                    {t.mode === "cash" ? "💵" : "🏦"}
                  </div>
                </div>
              </div>
        ...
        {total > 50 && (
          <div style={{ display: "flex", gap: 8, marginTop: 12, justifyContent: "center" }}>
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
              style={{ ...S.chip, opacity: page === 1 ? 0.4 : 1 }}>← Prev</button>
            <span style={{ padding: "6px 12px", fontSize: 13, color: C.slate500 }}>Page {page} of {Math.ceil(total / 50)}</span>
            <button onClick={() => setPage(p => p + 1)} disabled={page >= Math.ceil(total / 50)}
              style={{ ...S.chip, opacity: page >= Math.ceil(total / 50) ? 0.4 : 1 }}>Next →</button>
          </div>
        )}
      </div>
    </div>
