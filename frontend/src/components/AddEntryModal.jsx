...
const CATS = {
  in:  ["Bed Charge","Lab Test","Medicine Sale","OPD Fee","Admission","Consultation","Other"],
...
export default function AddEntryModal({ onClose, prefillBiz }) {
  ...
  const save = async () => {
    ...
    try {
      await api.post("/transactions", { businessId: bizId, type, amount: amt, mode, category: cat || null, description: desc || null });
      ...
      <div style={S.mBox}>
        ...
        {mode === "bank" && chosenBiz && (
          <div style={{ marginTop: 6, fontSize: 12, color: "#1E40AF", background: "#EFF6FF", borderRadius: 8, padding: "7px 10px" }}>
            {chosenBiz.icon} {chosenBiz.name} bank balance: <strong>{INR(chosenBiz.bankBalance)}</strong>
          </div>
        )}
