...
// Amount: plain number, "3k", "3000", "3 thousand", "1 lakh"
...
recog.onresult = (e) => {
  // Try all alternatives, use first that parses successfully
  let best = null;
...
try {
  await api.post("/transactions", {
    ...
    amount:      amt,
    mode:        parsed.mode,
    category:    parsed.mode === "bank" ? "Collection" : "Collection",
...
```
    ...
    {phase === "unsupported" && (
      <div style={{ background: "#FEF2F2", borderRadius: 12, padding: 16, textAlign: "center" }}>
        ...
        <div style={{ fontSize: 14, fontWeight: 700, color: C.red }}>Voice not supported</div>
        <div style={{ fontSize: 12, color: C.slate500, marginTop: 4 }}>
          Use Chrome or Safari on Android/iPhone.
        </div>
      </div>
    )}
    ...
    {(phase === "idle" || phase === "error") && (
      <div style={{ textAlign: "center" }}>
        ...
        {phase === "error" && (
          <div style={{ fontSize: 12, color: C.red, marginTop: 8 }}>{transcript}</div>
        )}
      </div>
    )}
    ...
    {phase === "listening" && (
      <div style={{ textAlign: "center" }}>
        <button onClick={stopListening}
          style={{ width: 90, height: 90, borderRadius: "50%", background: C.red, border: "4px solid #FCA5A5", fontSize: 36, cursor: "pointer", display: "inline-flex", alignItems: "center", justifyContent: "center", animation: "pulse 1s infinite" }}>
          🎙️
        </button>
    ...
    {(phase === "parsed" || phase === "saving") && parsed && (
      <div>
        ...
        {saveErr && (
          <div style={{ background: "#FEF2F2", color: C.red, borderRadius: 10, padding: "10px 14px", fontSize: 13, marginBottom: 12 }}>
            ⚠️ {saveErr}
          </div>
        )}
