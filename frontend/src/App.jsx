...
export default function App() {
  ...
  const openTransfer = (mode = "business") => { setTransferMode(mode); setShowTransfer(true); };
  const goTo = (s, biz = null) => { setActiveBiz(biz); setScreen(s); };
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100dvh", background: "#F8FAFC", position: "relative", paddingBottom: 72 }}>
      ...
      {/* FIX: null-guard activeBiz — prevents crash if screen is bizDetail but biz is null */}
      {screen === "bizDetail"    && activeBiz && <BizDetail biz={activeBiz} goBack={() => setScreen("dashboard")} openTransfer={openTransfer} />}
      {screen === "quickadd"     && <QuickAdd     openVoice={() => setShowVoice(true)} />}
      {screen === "transactions" && <Transactions />}
      {screen === "reports"      && <Reports />}
      {screen === "partners"     && <Partners     openTransfer={openTransfer} />}

      {/* FIX: FAB z-index 95 > NavBar z-index 90; safe-area-inset for iOS home indicator */}
      {screen === "quickadd" ? (
        <button onClick={() => setShowVoice(true)} style={{
          ...
          fontSize: 24, cursor: "pointer", zIndex: 95,
        ...
        <button onClick={() => setShowAdd(true)} style={{
          ...
          fontSize: 28, cursor: "pointer", zIndex: 95,
